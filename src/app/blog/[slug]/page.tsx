...
export default async function BlogPost({ params }: PageProps) {
    ...
    return (
        <div className="min-h-screen px-4 sm:px-10 pt-20 sm:pt-24 pb-12 font-[family-name:var(--font-geist-sans)] max-w-3xl mx-auto break-words">
            ...
            <header className="mb-10">
                ...
                <h1 className="text-[clamp(1.75rem,7vw,3rem)] font-black mb-6 text-balance leading-tight bg-clip-text text-transparent bg-linear-to-r from-cyan-200 via-blue-400 to-purple-400 drop-shadow-md">
                    {post.title}
                </h1>
            ...
            {/* Blog Content */}
            <div className="space-y-6 sm:space-y-8 mb-12 min-w-0">
                {(() => {
                    try {
                        let blocks: any[] = []
                        if (typeof post.content === 'string') {
                            ...
                            if (post.content.trim().startsWith('[') || post.content.trim().startsWith('{')) {
                                ...
                                return (
                                    <div className="prose prose-pre:overflow-x-auto prose-img:rounded-lg prose-img:max-w-full prose-table:block prose-table:overflow-x-auto prose-lg dark:prose-invert max-w-none prose-headings:font-bold prose-a:text-blue-600 hover:prose-a:underline">
                        ...
                        return blocks.map((block: any) => {
                            if (block.type === 'subheading') {
                                ...
                                if (level === 3) {
                                    return <h3 key={block.id} className={`text-lg sm:text-xl font-bold mt-6 mb-3 break-words ${neonGradient}`}>{block.content}</h3>
                                }
                                if (level === 4) {
                                    return <h4 key={block.id} className={`text-lg font-bold mt-4 mb-2 ${neonGradient}`}>{block.content}</h4>
                                }
                                return <h2 key={block.id} className={`text-xl sm:text-2xl font-bold mt-8 mb-4 break-words ${neonGradient}`}>{block.content}</h2>
                            }
                            if (block.type === 'paragraph') {
                                return <p key={block.id} className="text-gray-300 leading-relaxed text-base sm:text-lg break-words">{block.content}</p>
                            }
                            if (block.type === 'command') {
                                return (
                                    <div key={block.id} className="my-6 bg-black/50 rounded-lg overflow-hidden border border-gray-800">
                                        ...
                                        <div className="p-3 sm:p-4 font-mono text-xs sm:text-sm text-green-400 whitespace-pre-wrap break-all">
                                            $ {block.content}
                                        </div>
                                    </div>
                                )
                            ...
                            try {
                                ...
                                return refs.map((ref: any, idx: number) => (
                                    <li key={idx} className="flex gap-2 items-start">
                                        ...
                                        <a href={/^https?:\/\//i.test(String(ref.url)) ? ref.url : undefined} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300 hover:underline break-all">
                                            {ref.text || ref.url}
                                        </a>
                                    </li>
                                ))
