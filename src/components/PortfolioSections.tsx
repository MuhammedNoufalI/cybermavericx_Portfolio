...
import type { Portfolio, SectionKey } from '@/lib/sections'
...
const card = 'border border-purple-500/10 rounded-2xl p-6 hover:border-purple-500/40 hover:shadow-lg hover:shadow-purple-500/10 transition-all bg-[#120822]/60 backdrop-blur-md'
...
export default function PortfolioSections({ portfolio }: { portfolio: Portfolio }) {
            ...
            case 'about':
                return (
                    <SectionShell key={key} id="about" title="About" icon={<Quote className="text-blue-400" />}>
                        <div className="text-base sm:text-lg text-gray-400 leading-relaxed markdown-bio prose prose-invert max-w-none prose-p:my-2 prose-a:text-blue-400 prose-strong:text-white">
                            <ReactMarkdown rehypePlugins={[rehypeHighlight, rehypeRaw]}>{profile?.bio || ''}</ReactMarkdown>
                        </div>
                    </SectionShell>
                )
            case 'experience':
                                ...
                                <div key={job.id} className="ml-6 sm:ml-12 relative group">
                                    ...
                                    <div className={card}>
                                        <div className="flex flex-col sm:flex-row sm:items-baseline sm:justify-between mb-2">
                                            <h3 className="text-xl font-bold tracking-wide bg-clip-text text-transparent bg-linear-to-r from-pink-400 to-purple-400">{job.position}</h3>
                                        ...
                                        <div className="text-lg font-medium mb-3 flex items-center gap-2 bg-clip-text text-transparent bg-linear-to-r from-cyan-300 to-blue-400">
                                            {job.company}
                                            {job.location && <span className="text-xs text-gray-500 font-normal flex items-center ml-2 border-l border-gray-700 pl-2"><MapPin size={12} className="mr-1" />{job.location}</span>}
                                        </div>
                                        {job.description && <p className="text-gray-300 whitespace-pre-line leading-relaxed text-sm sm:text-base">{job.description}</p>}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {content.projects.map(p => (
                                <div key={p.id} className={`${card} flex flex-col`}>
                                    ...
                                    {p.description && <p className="text-gray-300 text-sm whitespace-pre-line leading-relaxed">{p.description}</p>}
                        ...
                        <div className="space-y-6">
                            {content.education.map(e => (
                                <div key={e.id} className={card}>
                                    ...
                                    {e.description && <p className="text-gray-300 text-sm mt-3 whitespace-pre-line">{e.description}</p>}
                                </div>
                            ))}
                        </div>
                    </SectionShell>
                    ...
                    <SectionShell key={key} id="certifications" title="Certifications" icon={<Award className="text-amber-500" />}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {content.certifications.map(cert => {
                                ...
                                const cls = 'p-6 rounded-xl bg-[#120822]/80 border border-purple-500/10 hover:border-purple-500/40 transition-all flex gap-4 items-start'
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {content.awards.map(a => (
                                <div key={a.id} className={card}>
                                    ...
                                    {a.description && <p className="text-gray-300 text-sm mt-3 whitespace-pre-line">{a.description}</p>}
                                </div>
                            ))}
                        </div>
                    </SectionShell>
            ...
            case 'custom':
                return content.custom.map(c => (
                    <SectionShell key={c.id} id={`custom-${c.id}`} title={c.title} icon={<span className="w-2 h-6 bg-purple-500 rounded-full" />}>
                        <div className="prose prose-invert max-w-none prose-a:text-blue-400">
                            <ReactMarkdown rehypePlugins={[rehypeHighlight, rehypeRaw]}>{c.content}</ReactMarkdown>
                        </div>
                    </SectionShell>
                ))
            case 'blog':
    ...
    const nodes = sections.filter(s => s.visible).map(s => render(s.key)).filter(Boolean)
    if (nodes.length === 0) return null

    return <div className="w-full flex flex-col gap-24">{nodes}</div>
}
